...
void BatchPrefillWithPagedKVCacheRun(TensorView float_workspace_buffer,
  ...
  DISPATCH_context(
      ...
      RaggedParams, PagedParams, [&] {
        ...
        params.q = static_cast<DTypeQ*>(q.data_ptr());
        paged_kv_t<typename PagedParams::DTypeK, IdType, typename PagedParams::DTypeV> paged_kv(
            num_kv_heads, page_size, HEAD_DIM_VO, batch_size, kv_layout,
            static_cast<typename PagedParams::DTypeK*>(paged_k_cache.data_ptr()),
            static_cast<typename PagedParams::DTypeV*>(paged_v_cache.data_ptr()),
            k_cache_strides.data(), v_cache_strides.data(),
